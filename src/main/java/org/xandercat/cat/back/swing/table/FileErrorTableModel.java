...
import org.xandercat.cat.back.swing.component.ExceptionDetailAction;
import org.xandercat.cat.back.swing.component.ObjectRemovable;
import org.xandercat.cat.back.swing.component.RemoveObjectAction;
...
public class FileErrorTableModel extends FileTableModel implements FileCopyListener, ObjectRemovable, ActionListener {
	...
	public void fileCopied(File from, File to, FileCopier.CopyResult result) {
		if (result == FileCopier.CopyResult.ERROR) {
			...
			cancelButton.setAction(new RemoveObjectAction(this, fileDataItem, Icons.CANCEL_ICON, CANCEL));	
			retryButton.setAction(new RemoveObjectAction(this, fileDataItem, Icons.RETRY_ICON, RETRY));
			detailButton.setAction(new ExceptionDetailAction(
					Icons.INFO_ICON, fileDataItem.getThrowable(), "Copy Error Detail: " + fileDataItem.getFile().getAbsolutePath()));
